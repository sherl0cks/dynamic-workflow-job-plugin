...
public class ReleasePipelineVisitor implements Visitor {
	...
	private void createBuildAppScript(Application app){
	...
	private void createListOfShellCommandsScript( Application app, String tool ){
		...
		if ( app.getScmRef() == null || app.getScmRef().isEmpty() ){
			throw new RuntimeException( "app.buildCommands cannot be empty. we are currently using scmRef, with comma delimited string for buildCommands. it's a hack and will be fixed soon." );
		} else {
